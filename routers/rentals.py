...
async def create_rental(rental: rental_schema.RentalCreate, session: AsyncSession = Depends(get_session)):
...
async def update_rental(rental_id: int, rental: rental_schema.RentalUpdate, session: AsyncSession = Depends(get_session)):
...
async def delete_rental(rental_id: int, session: AsyncSession = Depends(get_session)):
...
async def get_rentals(renter_id: Optional[int] = None, session: AsyncSession = Depends(get_session)):
...
async def get_rentals(name: str, session: AsyncSession = Depends(get_session)):
